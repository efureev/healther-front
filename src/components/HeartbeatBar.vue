...
export default {
  ...
  computed: {
    ...
    wrapStyle() {
      const topBottom = (((this.beatHeight * this.hoverScale) - this.beatHeight) / 2)
      const leftRight = (((this.beatWidth * this.hoverScale) - this.beatWidth) / 2)
    ...
    barStyle() {
      if (this.move && this.shortBeatList.length > this.maxBeat) {
        const width = -(this.beatWidth + this.beatMargin * 2)
